...
export const register = catchAsyncError(async (req, res, next) => {
  ...
  if (!name || !email || !password) {
    return next(new ErrorHandler("Please provides all the required fields", 400));
  }
  ...
  if (phone && !/^01[3-9]\d{8}$/.test(phone)) {
    return next(new ErrorHandler("Please provide a valid 11-digit BD phone number", 400));
  }
  ...
  await database.query("DELETE FROM users WHERE email = $1 AND is_verified = FALSE", [email]);
  ...
  try {
    ...
    await database.query("DELETE FROM users WHERE email = $1 AND is_verified = FALSE", [email]);
    return next(new ErrorHandler("Failed to send OTP email. Please try again.", 500));
  }
});
...
export const verifyOTP = catchAsyncError(async (req, res, next) => {
  ...
  if (userResult.rows.length === 0) {
    return next(new ErrorHandler("Invalid or expired OTP. Please try again.", 400));
  }
  ...
  sendToken(verifiedUser.rows[0], 201, "Account verified and registered successfully", res);
});
...
export const resendOTP = catchAsyncError(async (req, res, next) => {
  ...
  if (userResult.rows.length === 0) {
    return next(new ErrorHandler("No pending registration found for this email", 404));
  }
  ...
  try {
    await sendEmail({ email, subject: "Daily Bazar - New OTP Code", message: otpEmailTemplate });
...
export const login = catchAsyncError(async (req, res, next) => {
  ...
  const user = await database.query("SELECT * FROM users WHERE email = $1 AND is_verified = TRUE", [email]);
...
// Bug 19 FIX: don't leak user existence — return same message for both cases
